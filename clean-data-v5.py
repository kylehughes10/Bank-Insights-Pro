...
def parse_ubpr_report(report_path):
    ...
    data_dict = {}
    metrics_processed = set()  # To track processed metrics
    ...
    if date_line_index is not None and date_line_index < len(lines):
        ...
        dates = sorted(set(dates), key=lambda x: pd.to_datetime(x, format='%m/%d/%Y'), reverse=True)
...
report_path = 'C:/Users/kyleh/OneDrive/Documents/YouTube/Bank-Insights-Pro-main/Bank-Insights-Pro-main/UBPR Report.txt'
...
print(new_data_df.head())  # Displaying a snippet of the parsed data
